...
| `attest-green [--passed N]` | record "suite green at this checkout's exact HEAD" (clean tracked tree required) into the main root's `.comms/cache/suite-attest.log`; a green `tests/run.sh` records itself automatically |
