...
| `route [--task T\|--file F\|--current-tier T\|--context-tokens N\|--] <task>` | classify an `/auto` query: `plan: yes\|no`, implementer `effort`, and abstract `tier` (`fast\|balanced\|strong`). Policy is composed in code (jev-router style): prompt overrides win, low confidence lands on `balanced`, a large `--context-tokens` refuses a downgrade. Fail-open (`plan: no`, `effort: medium`, `tier: balanced`) with no key, on timeout, or on a malformed answer. Never selects a reviewer or a vendor model id. `COMMS_ROUTE=0` disables. Optional `COMMS_ROUTE_LOG` JSONL for calibration |
