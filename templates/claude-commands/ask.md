...
1. **Parse the argument — target agent, then question.**
   ...
   - A **review identity** (`"$COMMS_SH" agents --review` — e.g. a `claude-review` declared under `review-agents` in `.comms/config`) is a known name but REVIEW-ONLY: it cannot be consulted, and `send`, `comms.sh ask` and the ACP consult all refuse it. If the first word names one, send nothing: tell the user it only reviews, and offer the drivers (`"$COMMS_SH" agents --drivers`) — to consult the same model, `/ask` the driver that runs on it. Never re-read the word as part of the question.
