...
# Generated by the PRODUCTION function, not hand-copied. The previous fixture pasted the
# shim body inline, so it could only ever prove the copy behaved like itself -- and it did
# behave, while the shipped shim was bypassable three ways. Same defect as the copied
# findings parsers. (codex, round 6.)
GS_REAL="$WORK/gitshim-real"; mkdir -p "$GS_REAL"
printf '#!/bin/sh\necho "REAL-GIT-WOULD-RUN: $*"\n' > "$GS_REAL/git"; chmod +x "$GS_REAL/git"
( set -e
  eval "$(awk '/^write_git_shim\(\) \{/,/^\}/' "$REPO/helpers/runphase.sh")"
  write_git_shim "$GS" "$GS_REAL/git" )
...
gs_allowed() {  # <label> <args...> — a read-only verb must reach git
  local label="$1"; shift
  local out; out="$("$GS/git" "$@" 2>&1 | head -1)"
  case "$out" in
    "REAL-GIT-WOULD-RUN:"*) ok "shim passes $label through to git" ;;
    *) fail "shim BLOCKED read-only $label (got: ${out:0:70})" ;;
  esac
}
